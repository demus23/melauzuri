...
export async function POST(_req: Request, { params }: RouteContext) {
  try {
    ...
    if (order.product === "consultation") {
      ...
      consultation = await ConsultationCase.findOne({
        paymentOrderId: order._id,
      });

      if (!consultation) {
        consultation = await ConsultationCase.create({
          userId: user._id,
          paymentOrderId: order._id,
...
await sendEmail({
  ...
  html: paymentApprovedEmail(user.name || "there"),
});

if (order.product === "course" || order.product === "bundle") {
  await sendEmail({
    to: user.email,
    subject: "Your course access has been granted",
    html: courseAccessGrantedEmail(
      user.name || "there",
      order.product === "bundle" ? "Skincare Course Bundle" : "Skincare Course"
    ),
  });
}
